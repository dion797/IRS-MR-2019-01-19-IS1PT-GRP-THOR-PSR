...
function submitForm() {
  ...
  // preparing data
  var formData = {
    housetype:      $('#form-preference').find('select[name="housetype"]').val(),
    income:         $('#form-preference').find('select[name="income"]').val(),
    residence:      filter($('#form-preference').find('input[name="residence"]').val()),
    risk:           filter($('#form-preference').find('input[name="risk"]').val()),
    bill:           filter($('#form-preference').find('input[name="bill"]').val()),
    sd:             filter($('#form-preference').find('input[name="sd"]').val()),
    incentive:      filter($('#form-preference').find('input[name="incentive"]').val()),
    brand:          filter($('#form-preference').find('input[name="brand"]').val()),
    otherconsider:  filter($('#form-preference').find('input[name="otherconsider"]').val()),
  };

  // Ajax post
  $.ajax({ type: 'POST', url: '/api/choose', data: formData,
    success: function(data) {
      $('#form-preference').removeClass("loading");
      if (data.length == 0) {
        alert("datalength0")
    ...
    error: function() {
      confirm("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
      $('#outer-box').hide();
      saySomething(true, "Sorry some unexpected errors occured :()")
  }});
...
function renderCards2(data) {
  ...
  for (i = 0; i < data.length; i++) {
    content += `
      <div id="card-`+i+`" class="dish-card ui `+cardColors[index]+` card" style="display:block">
        <div class="image">
          <img src="/static/images/`+data[i].name+`.png" >
        </div>
        <div class="content">
          <div class="row" style="padding-left:10px">
            <div class="header left floated">`+data[i].plan+`</div>
          </div>
          <!--<div>`+data[i].name+`</div>-->
          <div class="row">
            <div class="col-sm">Price in cents/KwH or discount</div>
            <div class="col-sm">`+data[i].priceCentsKwh+`</div>
          </div>
          <div class="row">
            <div class="col-sm">Incentives</div>
            <div class="col-sm">`+data[i].incentives+`</div>
          </div>
          <div class="row">
            <div class="col-sm">Contract (months)</div>
            <div class="col-sm">`+data[i].contract+`</div>
          </div>
          <div class="row">
            <div class="col-sm">Automatic Renewal</div>
            <div class="col-sm">`+data[i].autoRenewal+`</div>
          </div>
          <div class="row">
            <div class="col-sm">Smart Meter Required</div>
            <div class="col-sm">`+data[i].smartMeterReq+`</div>
          </div>
          <div class="row">
            <div class="col-sm">Direct Billing</div>
            <div class="col-sm">`+data[i].separateBill+`</div>
          </div>
          <div class="row">
            <div class="col-sm">One-Time Registration Fee</div>
            <div class="col-sm">`+data[i].OTRegFee+`</div>
          </div>
          <div class="row">
            <div class="col-sm">Late Payment Charges</div>
            <div class="col-sm">`+data[i].lateCharge+`</div>
          </div>
          <div class="row">
            <div class="col-sm">Early Termination Charges</div>
            <div class="col-sm">`+data[i].earlyTerm+`</div>
          </div>
          <div class="row">
            <div class="col-sm">Security Deposit</div>
            <div class="col-sm">`+data[i].securityDeposit+`</div>
          </div>
          <div class="row">
            <div class="col-sm">Others</div>
            <div class="col-sm">`+data[i].others+`</div>
          </div>
        </div>
      </div>`;
